...
export async function createSingleTimeScheduleFromTransaction(
  ...
  // Name generated by the app, not typed by the user, so it is created in the
  // user's language. It is only ever displayed; lookups go through the id.
  const scheduleName = t(
    'Auto-created future transaction ({{formattedDate}}) - {{timestamp}}',
    { formattedDate, timestamp: String(timestamp) },
  );
